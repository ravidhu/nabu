...
const URL_MICROPHONE: &str = "x-apple.systempreferences:com.apple.preference.security?Privacy_Microphone";
...
pub fn run(model: &str, diarizer: &str) -> i32 {
    ...
    for check in &checks {
        ...
        println!("  {:<6} {:<28} {}", tag, check.name, check.detail);
        ...
        Ok(out) if out.status.success() => {
            ...
            if major >= 13 {
                Check {
                    name: "macOS version",
                    level: Level::Ok,
                    detail: format!("{ver} (≥ 13.0 required)"),
                    remediation: None,
                }
            } else {
                Check {
...
fn remediation_screen_recording() -> String {
    ...
    let (target, quit_line, header_note) = match &app {
        Some(terminal) => (
            format!("{} (drag it from {})", terminal.name, terminal.path),
            format!("Quit {} completely (⌘ + Q) and reopen it", terminal.name),
            format!("How to fix (detected terminal: {})", terminal.name),
        ),
        None => (
            "your terminal app (Terminal lives at /System/Applications/Utilities/Terminal.app, \
             most others at /Applications/<name>.app)"
                .to_string(),
...
fn check_microphone() -> Check {
    ...
    match device.default_input_config() {
        Ok(_) => Check {
            name: "microphone",
            level: Level::Ok,
            detail: "default input device available".into(),
            remediation: None,
        },
        Err(err) => Check {
            name: "microphone",
            level: Level::Fail,
            detail: format!("cannot open input ({err})"),
...
fn check_nabu_home() -> Check {
    ...
    if !paths.transcribe.join(".venv").exists() {
        return Check {
            name: "~/.nabu/transcribe",
            level: Level::Fail,
...
fn check_uv() -> Check {
    ...
    let uv = paths.as_ref().map(|paths| paths.uv.clone());
    let Some(uv) = uv else {
        return Check {
            name: "uv runtime",
            level: Level::Warn,
            detail: "could not resolve path".into(),
            remediation: None,
        };
    };
    if !uv.exists() {
    ...
    match diarizer {
        "wespeaker" => {
            if models::wespeaker_cached() {
                Check {
                    name: "diarizer (wespeaker)",
                    level: Level::Ok,
                    detail: "model present".into(),
                    remediation: None,
                }
            } else {
                Check {
        ...
        "pyannote" => {
            if models::pyannote_cached() {
                Check {
                    name: "diarizer (pyannote)",
                    level: Level::Ok,
                    detail: "models present".into(),
                    remediation: None,
                }
            } else {
                Check {
                    ...
                    remediation: Some(
                        "Accept terms on HuggingFace, then: nabu --setup --hf-token <TOKEN>".into(),
                    ),
                }
            }
        }
...
fn check_disk_space() -> Check {
    ...
    if free_mb == 0 {
        Check {
            name: "disk space",
            level: Level::Warn,
            detail: "could not determine".into(),
            remediation: None,
        }
        ...
        Check {
            ...
            level: Level::Warn,
            detail: format!(
                "{} MB free on home volume — recordings may fill the disk",
                free_mb
            ),
